...
module.exports = function(app, options, cb) {
  ...
  app.questions.disable('save')
    .set('global.preferences', 'Would you like to use global preferences?', {
      type: 'confirm',
      next: function(answer, question, answers, next) {
        ...
        if (answer === true && app.store.has('preferences')) {
          ...
          var preferences = app.store.get('preferences');
          options.answers = utils.merge({}, answers, preferences);
          ...
          if (opts.force === true) {
            app.ask('config', options, next);
          } else {
            next(null, options.answers);
          }
  ...
  app.ask('global.preferences', function(err, answers) {
    ...
    if (answer === true) {
      app.store.set('preferences', answers.config);
    }

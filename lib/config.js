...
/**
 * Normalize the config object to be written to
 * either `app.store.data` or the user's local
 * config. Local config is usually a property
 * on package.json, but may also be a separate
 * file.
 */

module.exports = function(app, config, existing) {
  var opts = app.options || {};
  opts.keys = ['toc', 'layout', 'plugins', 'related', 'reflinks'];
